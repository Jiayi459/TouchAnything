"""Seasonal-naive baseline — fork of src/actionsense/eval_harness/baselines/seasonal.py.

FORKED, NOT SHARED (see ../masking.py's docstring). The only change: the output array
`np.empty((H, 6))` becomes `np.empty((H, len(self.cfg.channels)))`. The period-estimation
algorithm and causal-index recursion are unchanged.

y_hat[t+h] = y at the same phase, whole periods back:

    idx = (t + h) - ceil(h / T) * T          (always <= t  =>  strictly causal)

The period T is estimated PER GROUP (object_category) from the dominant peak of the TRAIN
...
Estimation uses TRAIN only. Autocorrelation is computed per recording and averaged (no
cross-recording concatenation -> no boundary artifacts).

Kept in the OpenTouch v1 pass for parity with ActionSense (SESSION_LOG OQ-E): ActionSense's
seasonal-naive was found INERT (all 5 groups fell back to persistence, no autocorrelation
peak in 0.3-3s). OpenTouch's clips are shorter still, so the same null result is a plausible
and worthwhile replication check, not a load-bearing model.
"""
