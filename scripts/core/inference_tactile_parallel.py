...
# ──────────────────────────────────────────────
@torch.no_grad()
def infer_trajectory_batched(hdf5_path: str, model, device, cfg, pose_source: str,
    ...
    Args:
        ...
        pose_source: pose source, one of 'rokoko', 'wilor', or 'hamer'
    ...
    # Pose-valid frame: both hands valid for the selected source
...
def make_comparison_video(ego_frames, left_frames, right_frames,
                          ...
                          vmax: float = 1.0):
    """Compose comparison video with 2D heatmaps for pressure

    Encode video with cv2.VideoWriter without depending on ffmpeg.
    """
    T = ego_frames.shape[0]
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)

    # Encode video with cv2.VideoWriter
    # mp4v is OpenCV default MPEG-4 encoder and does not require external ffmpeg
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video_writer = cv2.VideoWriter(str(output_path), fourcc, float(fps),
                                   (VIDEO_W, VIDEO_H))
    if not video_writer.isOpened():
        raise RuntimeError(f'cv2.VideoWriter initialization failed: {output_path}')
...
def process_single_trajectory(args):
    """Single-trajectory processing function for multiprocessing"""
    hdf5_path, output_dir, fps, batch_size, views, pose_source, skip_video, skip_hdf5 = args
    ...
    # Compute metrics over all frames with predictions
...
def main():
    ...
    pose_source = cfg['data'].get('pose_source')
    if pose_source not in {'rokoko', 'wilor', 'hamer'}:
        raise ValueError(
            "Please set the pose source in config data.pose_source. Options: 'rokoko', 'wilor', 'hamer'"
        )
        ...
        with tqdm(total=len(task_args), desc='Overall progress') as pbar:
            for task_arg in task_args:
                try:
                    ...
                    (task_name, traj_name, metrics_all,
                     valid_pred, total) = result
                    results.append(result)
                    print(
                        f'\n[{task_name}/{traj_name}] '
                        f'(valid prediction frames={valid_pred}/{total})'
                    )
                    print(f'  Temporal Acc: {metrics_all["temporal_accuracy"]:.4f}  '
    ...
    if results:
        ...
        total_valid_pred = int(sum(r[3] for r in results))
        total_frames = int(sum(r[4] for r in results))

        print(f'\n========== Summary Statistics ==========')
        print(f'Processed trajectories: {len(results)}')
        print(f'Total valid prediction frames: {total_valid_pred}/{total_frames}')
        print(f'\nPaper metrics (PressureVision):')
        ...
        with open(results_file, 'w', encoding='utf-8') as f:
            ...
            for (task_name, traj_name, metrics_all,
                 valid_pred, total) in results:
                f.write(f'[{task_name}/{traj_name}]\n')
                f.write(f'  valid prediction frames: {valid_pred}/{total}\n')
                f.write(f'  Temporal Accuracy: {metrics_all["temporal_accuracy"]:.4f}\n')
                f.write(f'  Contact IoU:       {metrics_all["contact_iou"]:.4f}\n')
                f.write(f'  Volumetric IoU:    {metrics_all["volumetric_iou"]:.4f}\n')
                f.write(f'  MAE:               {metrics_all["mae"]:.4f}\n')
                f.write(f'  MAE Left:          {metrics_all["mae_left"]:.4f}\n')
                f.write(f'  MAE Right:         {metrics_all["mae_right"]:.4f}\n')
                f.write('\n')
