"""Metrics — fork of src/actionsense/eval_harness/metrics.py.

FORKED, NOT SHARED (see masking.py's docstring for why). The only change: `.reshape(-1, 6)`
becomes `.reshape(-1, mask.shape[-1])` — these functions were already cfg-free / pure array
ops, so the fix is shape-driven rather than needing a config parameter threaded in. The
scoring DEFINITIONS (MSE, MAE, skill, normalized RMSE) are byte-identical to the original.

All predictions are shaped (N, H, C): N forecast origins, H horizon steps (indexed by
TARGET time t+h, h = 1..H), C channels. `mask` is the same shape (from masking.valid_mask,
...
def masked_channel_mse(ytrue: np.ndarray, yhat: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """(N,H,C) -> (C,): MSE per channel, aggregated over all valid origins & horizons."""
    e = (yhat - ytrue) ** 2
    C = mask.shape[-1]
    num = (e * mask).reshape(-1, C).sum(0)
    den = mask.reshape(-1, C).sum(0)
    return num / np.maximum(den, 1.0)


def masked_horizon_mse(ytrue: np.ndarray, yhat: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """(N,H,C) -> (H,C): MSE per (horizon step, channel)."""
    e = (yhat - ytrue) ** 2
    num = (e * mask).sum(0)          # (H,C)
    den = mask.sum(0)                # (H,C)
    return num / np.maximum(den, 1.0)
...
def masked_horizon_mae(ytrue: np.ndarray, yhat: np.ndarray, mask: np.ndarray) -> np.ndarray:
    """(N,H,C) -> (H,C): MAE per (horizon step, channel)."""
    e = np.abs(yhat - ytrue)
