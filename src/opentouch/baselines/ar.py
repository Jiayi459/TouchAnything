"""Linear AR baseline — fork of src/actionsense/eval_harness/baselines/ar.py.

FORKED, NOT SHARED (see ../masking.py's docstring). The only change: every place a
coefficient/output array was shaped with a literal 6 now uses `len(self.cfg.channels)`.
The fitting algorithm, order selection, and iterated-forecast recursion are unchanged.
...
class AR(Baseline):
    ...
    def __init__(self, cfg, norm):
        ...
        # coef[group][order] -> (n_ch, p+1) = [phi_1..phi_p, b] per channel
    ...
    def fit(self, train: dict[int, np.ndarray], groups: dict[int, str]) -> None:
        for g, recs in by_group(train, groups).items():
            z = np.concatenate([self.norm.z(Y) for Y in recs.values()], axis=0)   # (sumT,n_ch)
            self.coef[g] = {}
            for p in self.orders:
                C = np.zeros((self.n_ch, p + 1))
    ...
    def predict(self, hist: np.ndarray, H: int, group: str) -> np.ndarray:
        p = self.order[group]
        C = self.coef[group][p]                       # (n_ch, p+1)
        z = self.norm.z(hist)
        phi, b = C[:, :p], C[:, p]
        buf = list(z[-p:]) if len(z) >= p else list(np.zeros((p - len(z), self.n_ch))) + list(z)
        out = np.empty((H, self.n_ch))
        for h in range(H):
            recent = np.stack(buf[-p:])[::-1]          # (p,n_ch): lag1..lagp
